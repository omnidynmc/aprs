 ...
 ******************************/
  ...
  const bool UnitTest::run() {
    ...
    _encodeUncompressedMoving();
    _encodeObjectMoving();
    _encodeCompressedMoving();
    _encodeMessage();
  ...
  void UnitTest::_decodeUncompressedMovingDfr() {
    ...
    stringMap["aprs.packet.dirspd.speed"] = "18.52";
    stringMap["aprs.packet.dirspd.direction"] = "036";
    stringMap["aprs.packet.dfr.bearing"] = "293";
    stringMap["aprs.packet.dfr.range"] = "411.99";
    stringMap["aprs.packet.dfr.hits"] = "8";
    stringMap["aprs.packet.dfr.quality"] = "6";

    _test(packet, stringMap, "basic uncompressed packet - moving target w/ dfr");

    stringMap.clear();

    packet = _createPacketSentence("WB4APR-3", "APRX76,WIDE2-2,qAR,N3UJJ", ";",
                                   "DF3      *021736z350 .  N/0752 .  W\\"
                                   "001/000/055/968",
                                   "/DFReport",
                                   stringMap);
    ...
    stringMap["aprs.packet.position.latitude.decimal"] = "35.083332";
    stringMap["aprs.packet.position.longitude.decimal"] = "-75.41666";
    stringMap["aprs.packet.position.ambiguity"] = "3";

    stringMap["aprs.packet.dirspd"] = "001/000";
    stringMap["aprs.packet.dirspd.speed"] = "0.00";
    stringMap["aprs.packet.dirspd.direction"] = "001";
    stringMap["aprs.packet.dfr.bearing"] = "055";
    stringMap["aprs.packet.dfr.range"] = "823.98";
    stringMap["aprs.packet.dfr.hits"] = "9";
    stringMap["aprs.packet.dfr.quality"] = "8";

    _test(packet, stringMap, "basic uncompressed packet - moving target w/ dfr (real world)");
...
//    stringMap["aprs.packet.weather.wind.gust"] = "0";
  ...
  const std::string UnitTest::_createPacketSentence(const std::string &source, const std::string &path, const std::string &command, const std::string &content,
                                               const std::string &comment, stringMapType &stringMap) {
    return _createPacketSentence(source, path, command, content, comment, "", stringMap);
  } // _createPacketSentence

  const std::string UnitTest::_createPacketSentence(const std::string &source, const std::string &path, const std::string &command, const std::string &content,
                                               const std::string &comment, const std::string &suffix, stringMapType &stringMap) {
    openframe::StringToken p;
    std::string packet;
    std::stringstream s;
    size_t i;
  ...
  const bool UnitTest::_test(const std::string &parseMe, stringMapType &stringMap, const std::string &testName) {
    APRS *a;
    bool isOK = true;
    ...
    try {
      ...
      std::cout << " not ok - " << testName << " failed to parse: " << testName << "; " << e.message() << std::endl;
      ok(false);
      return false;
    } // catch
    ...
    for(ptr = stringMap.begin(); ptr != stringMap.end(); ptr++) {
      if (!a->isString(ptr->first)) {
        std::cout << " not ok - " << testName << " [key: " << ptr->first << "] is missing" << std::endl;
        isOK = false;
        continue;
      } // if

      if (ptr->second != a->getString(ptr->first)) {
        std::cout << " not ok - " << testName << " [key: " << ptr->first << "] (" << ptr->second << ") != (" << a->getString(ptr->first) << ")" << std::endl;
        isOK = false;
      } // if
    } // for
  ...
  void UnitTest::_encodeUncompressedMoving() {
    Position *pos = new Position("NV6G", "APRS", 62.892, 27.6578, '/', '>', 36, 18.52,
                                 141.73,  0, "this is a test/A=000465");
    ...
    std::string pac = pos->compile();

    std::cout << pac << std::endl;

    std::stringstream s;
    s << "NV6G>APRS,W6CO-5,OH7AA-1*,WIDE2-1,qAR,OH7AA:"
      << "=6253.52N/02739.47E>036/010"
      << "this is a test/A=000465";

    std::cout << s.str() << std::endl;
  } // UnitTest::_encodeUncompressedMoving

  void UnitTest::_encodeObjectMoving() {
    Object *obj = new Object("NV6G", "APRS", "TEST", 62.892, 27.6578, '/', '>', 36, 18.52,
                             141.73,  0, "this is a test/A=000465", false, time(NULL) );
    ...
    std::stringstream s;
    s << "NV6G>APRS,W6CO-5,OH7AA-1*,WIDE2-1,qAR,OH7AA:;TEST     *"
      << "6253.52N/02739.47E>036/010"
      << "this is a test/A=000465";

    std::cout << s.str() << std::endl;
  } // UnitTest::_encodeObjectMoving

  void UnitTest::_encodeCompressedMoving() {
    Position *pos = new Position("KE6SAR-10", "APZMDR", 60.358234, 24.808378, '/', '>', 0, 57.23,
                                 0,  0, "Tero, Green Volvo 960, GGL-880");

    pos->add_digi("WIDE3-2")
        .add_digi("qAo")
        .add_digi("OH2MQK-1");

    std::string pac = pos->compile_compressed();

    std::cout << pac << std::endl;

    std::stringstream s;
    s << "KE6SAR-10>APZMDR,WIDE3-2,qAo,OH2MQK-1:"
      << "!//zPHTfVv>!V_"
      << "Tero, Green Volvo 960, GGL-880";

    std::cout << s.str() << std::endl;
  } // UnitTest::_encodeCompressedMoving

  void UnitTest::_encodeMessage() {
    Message *msg = new Message("NV6G", "APRS", "AE6VQ", "This is a test message");
    ...
    std::stringstream s;
    s << "NV6G>APRS,W6CO-5,OH7AA-1*,WIDE2-1,qAR,OH7AA:"
      << ":AE6VQ    :This is a test message";

    std::cout << s.str() << std::endl;
  } // UnitTest::_encodeMessage
} // namespace aprs
